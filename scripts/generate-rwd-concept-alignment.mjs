...
const conceptIdsByModule = new Map(
  graphs
    ...
    .map((moduleId) => [
      moduleId,
      concepts
        .filter((concept) => concept.moduleId === moduleId)
...
const requireKnown = (context, ids) => {
  for (const id of ids) {
    if (!conceptById.has(id))
      throw new Error(`${context} references unknown concept ${id}.`);
  }
  return unique(ids);
};
...
for (const [moduleId, ids] of Object.entries(sourceModuleConcepts)) {
  sourceModuleConcepts[moduleId] = requireKnown(
...
const blockConcepts = {
  ...
  "lecture-best-practices-for-styling-forms":
    sourceModuleConcepts["styling-forms"],
  "lecture-working-with-css-transforms-overflow-and-filters":
    sourceModuleConcepts["css-box-model"],
  ...
  "lecture-best-practices-for-accessibility-and-css":
    sourceModuleConcepts["css-and-accessibility"],
  ...
  "lecture-working-with-attribute-selectors":
    sourceModuleConcepts["attribute-selectors"],
  "lecture-best-practices-for-responsive-web-design":
    sourceModuleConcepts["responsive-design"],
...
const alignments = reference.chapters.flatMap((chapter) =>
  chapter.modules.flatMap((sourceModule) => {
    if (!sourceModuleConcepts[sourceModule.id])
      throw new Error(
        `Missing concept map for source module ${sourceModule.id}.`,
      );
    return sourceModule.blocks.map((block) => {
      ...
      const conceptIds =
        mappingBasis === "block-specific-source"
          ...
          : [];
      const challengeIds = block.challengeOrder.map(
        (challenge) => challenge.id,
      );
      const challengeEvidence = block.challengeOrder.map(
        (challenge) => challenge.sourceEvidence,
      );
      ...
      return {
        ...
        sourceEvidence: {
          challengeIds,
          sourceFileSha256s: challengeEvidence.map(
            (evidence) => evidence.sha256,
          ),
          sourceBytes: challengeEvidence.reduce(
            (total, evidence) => total + evidence.bytes,
            0,
          ),
...
const benchmarkConceptIds = new Set(
  alignments.flatMap((alignment) => alignment.conceptIds),
);
...
const explicitModernExtensionConceptIdSet = new Set(
  explicitModernExtensionConceptIds,
);
...
if (extensionConcepts.length !== explicitModernExtensionConceptIds.length) {
  throw new Error(
    "Explicit modern extension inventory references an unknown concept.",
  );
}
const unresolvedConcepts = concepts.filter(
  (concept) =>
    !benchmarkConceptIds.has(concept.id) &&
    !explicitModernExtensionConceptIdSet.has(concept.id),
);
const extensionModuleIds = unique(
  extensionConcepts.map((concept) => concept.moduleId),
);
const courseExtensions = extensionModuleIds.map((moduleId) => {
  const moduleConceptRecords = extensionConcepts.filter(
    (concept) => concept.moduleId === moduleId,
  );
  return {
...
const sourceObjectiveIds = reference.chapters.flatMap((chapter) =>
  chapter.modules.flatMap((sourceModule) =>
    sourceModule.blocks.map((block) => block.objectiveId),
  ),
);
if (
  ...
  throw new Error(
    "Generated alignment objective order differs from the pinned source snapshot.",
  );
}
