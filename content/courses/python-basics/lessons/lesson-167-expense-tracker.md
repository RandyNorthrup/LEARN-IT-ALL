...
def create_transaction(id, type, amount, category, description, date=None):
    """Create a financial transaction dictionary"""
    return {
        "id": id,
        "type": type,
        "amount": amount,
...
def create_budget(category, amount, month):
    """Create a monthly budget dictionary for a category"""
    return {
        "category": category,
        "amount": amount,
        "month": month
...
def create_tracker(data_file="expenses.json"):
    """Create an expense tracker state dictionary"""
    tracker = {
        "data_file": data_file,
        "transactions": [],
        "budgets": []
...
def add_transaction(tracker, type, amount, category, description, date=None):
    """Add new transaction"""
    if amount <= 0:
...
def remove_transaction(tracker, transaction_id):
    """Remove transaction by ID"""
    for i, trans in enumerate(tracker["transactions"]):
        if trans["id"] == transaction_id:
            tracker["transactions"].pop(i)
            save_data(tracker)
            return True
    return False

def get_transactions(tracker, type=None, category=None,
                     start_date=None, end_date=None):
    """Get filtered transactions"""
    results = tracker["transactions"].copy()
...
def set_budget(tracker, category, amount, month=None):
    ...
    tracker["budgets"] = [b for b in tracker["budgets"]
                          if not (b["category"] == category and b["month"] == month)]

    budget = create_budget(category, amount, month)
    tracker["budgets"].append(budget)
    save_data(tracker)
    return budget

def get_budget(tracker, category, month=None):
...
def calculate_balance(tracker):
    ...
    income = sum(t["amount"] for t in tracker["transactions"]
                 if t["type"] == TRANSACTION_INCOME)
    expenses = sum(t["amount"] for t in tracker["transactions"]
                   if t["type"] == TRANSACTION_EXPENSE)
    return income - expenses

def get_spending_by_category(tracker, month=None):
    ...
    for trans in tracker["transactions"]:
...
def get_budget_status(tracker, month=None):
    """Get budget vs actual spending"""
    if not month:
        month = datetime.now().strftime("%Y-%m")

    spending = get_spending_by_category(tracker, month)
    status = []
...
def get_categories(tracker, type=None):
...
def save_data(tracker):
    """Save to JSON file"""
    data = {
        "transactions": [transaction_to_dict(t) for t in tracker["transactions"]],
        "budgets": [b for b in tracker["budgets"]]
    }
...
def load_data(tracker):
    """Load from JSON file"""
    if not Path(tracker["data_file"]).exists():
    ...
    try:
        with open(tracker["data_file"], 'r') as f:
            data = json.load(f)
            tracker["transactions"] = [transaction_from_dict(t)
                                       for t in data.get("transactions", [])]
            tracker["budgets"] = [budget_from_dict(b)
...
def monthly_summary(tracker, month=None):
    ...
    transactions = get_transactions(tracker, start_date=start, end_date=end)
    ...
    # Budget status
    budget_status = get_budget_status(tracker, month)
    if budget_status:
        report += f"\nBudget Status:\n"
        report += f"{'-'*60}\n"
...
def category_analysis(tracker, category, months=3):
    ...
    for i in range(months):
        ...
        # Get spending
        spending = get_spending_by_category(tracker, month_str)
...
def recent_transactions(tracker, count=10):
    """Show recent transactions"""
    transactions = get_transactions(tracker)[:count]
    ...
    for trans in transactions:
        ...
        type_str = "+" if trans["type"] == TRANSACTION_INCOME else "-"
...
def view_transactions(tracker):
    ...
    if choice == "1":
        transactions = get_transactions(tracker)
    elif choice == "2":
        transactions = get_transactions(tracker, type=TRANSACTION_INCOME)
    elif choice == "3":
        transactions = get_transactions(tracker, type=TRANSACTION_EXPENSE)
    elif choice == "4":
        category = input("Category: ").strip()
        transactions = get_transactions(tracker, category=category)
    else:
    ...
    for trans in transactions[:20]:  # Show last 20
        date_str = trans["date"].strftime("%Y-%m-%d")
        type_str = "Income" if trans["type"] == TRANSACTION_INCOME else "Expense"

        print(f"{date_str:<12} {type_str:<8} ${trans['amount']:<11.2f} "
              f"{trans['category']:<15} {trans['description']}")

def show_budget_status(tracker):
    """Display budget status"""
    status = get_budget_status(tracker)
    ...
    while True:
        ...
        try:
            if choice == "1":
                ...
                set_budget(tracker, category, amount)
                print(f"✓ Set budget for {category}: ${amount:.2f}")
            elif choice == "5":
                report = monthly_summary(tracker)
                print(report)
            elif choice == "6":
                category = input("\nCategory: ").strip()
                report = category_analysis(tracker, category)
                print(report)
            elif choice == "7":
...
def test_add_income(tmp_path):
    tracker = create_tracker(str(tmp_path / "test.json"))
    ...
    assert trans["type"] == TRANSACTION_INCOME

def test_calculate_balance(tmp_path):
    tracker = create_tracker(str(tmp_path / "test.json"))

    add_transaction(tracker, TRANSACTION_INCOME, 2000, "Salary", "")
    add_transaction(tracker, TRANSACTION_EXPENSE, 500, "Rent", "")
    add_transaction(tracker, TRANSACTION_EXPENSE, 200, "Food", "")

    balance = calculate_balance(tracker)
...
def test_budget_tracking(tmp_path):
    ...
    set_budget(tracker, "Food", 500.0, month)
    add_transaction(tracker, TRANSACTION_EXPENSE, 300, "Food", "Groceries")

    budget = get_budget(tracker, "Food", month)
    assert budget["amount"] == 500.0

    status = get_budget_status(tracker, month)
    assert len(status) == 1
