...
1. Your selector should target links inside list items.
2. You should give the `a` elements inside list items a `color` property.
3. const usedSelector = code.match(/^\s*?(.*?a)\s*?{/m)[1];
assert.notEmpty(helper.getStyle(usedSelector.replaceAll(/\s+/g, ' ').trim())?.color);
4. You should only change the color of `a` elements inside list elements.
5. You should give the `color` property a contrast with the background of at least 7:1. _Hint: I would use `#dfdfe2`_
6. for (let elem of document.querySelectorAll('li > a')) {
...
*Original Challenge ID: 614796cb8086be482d60e0ac*
