...
1. You should use the `footer, footer a` selector.
2. You should set the `color` to a value with a contrast ratio of at least `7:1`. _Hint: I suggest `#dfdfe2`_
3. const foot = getComputedStyle(document.querySelector('footer'))?.color;
const a = getComputedStyle(document.querySelector('footer a'))?.color;
4. const rgbFoot = foot?.match(/(\d+),\s(\d+),\s(\d+)/);
...
*Original Challenge ID: 6148e161ecec9511941f8833*
