...
1. The `#box-1` element should have the `flex` property set to a value of `2 2 150px`.
2. assert.equal(flexGrow, '2');
assert.equal(flexShrink, '2');
assert.equal(flexBasis, '150px');
3. The `#box-2` element should have the `flex` property set to a value of `1 1 150px`.
4. assert.equal(flexGrow, '1');
assert.equal(flexShrink, '1');
assert.equal(flexBasis, '150px');
5. Your code should use the `flex` property for `#box-1` and `#box-2`.
...
*Original Challenge ID: 587d78ae367417b2b2512afe*
