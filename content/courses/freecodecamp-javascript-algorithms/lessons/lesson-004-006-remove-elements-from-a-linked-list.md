...
1. Your `LinkedList` class should have a `remove` method.
2. Your `remove` method should reassign `head` to the second node when the first node is removed.
3. Your `remove` method should decrease the `length` of the linked list by one for every node removed.
4. Your `remove` method should reassign the reference of the previous node of the removed node to the removed node's `next` reference.
5. Your `remove` method should not change the linked list if the element does not exist in the linked list.
...
*Original Challenge ID: 587d8251367417b2b2512c63*
