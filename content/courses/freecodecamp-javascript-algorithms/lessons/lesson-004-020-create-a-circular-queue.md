...
1. The `enqueue` method should add items to the circular queue.
2. You should not enqueue items past the read pointer.
3. The `dequeue` method should dequeue items from the queue.
4. After an item is dequeued, its position in the queue should be reset to `null`.
5. Trying to dequeue past the write pointer should return `null` and does not advance the write pointer.
...
*Original Challenge ID: 587d8255367417b2b2512c75*
