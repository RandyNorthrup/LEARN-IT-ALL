...
1. You should have a `detectFullHouse` function.
2. Your `detectFullHouse` function should have a parameter.
3. When a full house is rolled, your `detectFullHouse` function should enable the third radio button, set its value to `25`, and set the third `span` to display the text `, score = 25`.
4. When a full house is not rolled, your `detectFullHouse` function should enable the final radio button, set its value to `0`, and set the final `span` to display the text `, score = 0`.
5. You should call your `detectFullHouse` function when your `rollDiceBtn` is clicked.
6. Math.random = myMathRandom;
try {
  rollDiceBtn.click();
} finally {
  Math.random = origMathRandom;
}
7. assert.isFalse(scoreInputs[2].disabled);
...
*Original Challenge ID: 657e0c2c6a9d37705146f34d*
