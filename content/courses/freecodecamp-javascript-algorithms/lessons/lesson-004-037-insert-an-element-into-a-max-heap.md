...
2. `MaxHeap` should have a method called `insert`.
3. `MaxHeap` should have a method called `print`.
4. The `insert` method should add elements according to the max heap property.
5. return (result.length == 6) ? (JSON.stringify(result) == solutionWithoutNull) : (JSON.stringify(result) == solution);
  })()
);
...
*Original Challenge ID: 587d825a367417b2b2512c8a*
