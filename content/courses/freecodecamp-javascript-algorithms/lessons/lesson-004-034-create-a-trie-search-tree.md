...
1. The `Trie` should have an `add` method.
2. The `Trie` should have a `print` method.
3. The `Trie` should have an `isWord` method.
4. The `print` method should return all items added to the trie as strings in an array.
5. The `isWord` method should return `true` only for words added to the trie and `false` for all other words.
...
*Original Challenge ID: 587d8259367417b2b2512c84*
