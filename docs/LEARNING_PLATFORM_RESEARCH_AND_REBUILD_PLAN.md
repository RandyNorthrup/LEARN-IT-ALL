...
The [semantic-text step design](research/courses/responsive-web-design-html-semantic-text-step-design.json) adds 209 explicit interactions: 18 model, 72 workshop, 27 faded, 18 debug, 27 lab, 20 delayed retrieval/correction review, and 27 no-hint assessment. It retrieves heading hierarchy, prose boundaries, content models, and destinations while teaching meaning-driven emphasis and voice; exact quotation, attribution, citation, and visible-source roles; readable abbreviation expansion; page or article contact scope; valid machine date, time, offset, week, year, and duration values; conventional subscript and superscript boundaries; code roles versus preserved whitespace; distinct annotation, stale-content, edit, relevance, and presentation semantics; and exact ruby base, annotation, language, fallback, copy, and linearization relationships. One hundred ninety-nine interactions require changed cases, every learner action is exact-unique, and all thirteen new or retrieved concepts appear as primary evidence in every family. Together the first nine designs cover 63 of 266 families and 1,324 planned interactions with 1,082 changed cases. They are reviewable planning structure, not learner-facing copy, source or language fixtures, time and ruby simulators, executable checks, canonical answers, or authored publication. The other 203 activity families remain undecomposed.
