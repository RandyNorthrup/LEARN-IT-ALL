...
The [images, graphics, and rights step design](research/courses/responsive-web-design-html-images-graphics-and-rights-step-design.json) adds 140 explicit interactions: 12 model, 48 workshop, 18 faded, 12 debug, 18 lab, 14 delayed retrieval/correction review, and 18 no-hint assessment. It separates externally replaced resource content from document children and page-controlled boxes, classifies image alternatives by contextual purpose, distinguishes intrinsic pixels and ratio hints from rendered dimensions and loading evidence, keeps figure captions separate from alternatives and source credits, requires exact ownership/permission/license/attribution/adaptation/commercial/share-alike evidence, refuses or escalates uncertain rights claims, and chooses SVG embedding from purpose, ownership, styling, caching, interaction, security, fallback, and accessibility boundaries. One hundred thirty-four interactions require changed cases.

The [media and embeds step design](research/courses/responsive-web-design-html-media-and-embeds-step-design.json) adds 72 explicit interactions: 6 model, 24 workshop, 9 faded, 6 debug, 9 lab, 9 delayed retrieval/correction review, and 9 no-hint assessment. It builds user-controlled native audio and video with source-selection, boolean-attribute, loading, poster, dimension, keyboard, state, failure, and fallback evidence; selects timed alternatives from audio-only, video-only, synchronized, live, prerecorded, interactive, language, and task context; verifies caption tracks, synchronized speech and non-speech cues, transcripts, and essential visual information; and refuses unnecessary embeds or constrains iframe purpose titles, origins, sandbox restrictions, feature permissions, referrer exposure, loading, navigation, privacy, responsive boxes, fallbacks, and diagnostic limitations. All 72 interactions require changed cases.

The [semantic-text step design](research/courses/responsive-web-design-html-semantic-text-step-design.json) adds 209 explicit interactions: 18 model, 72 workshop, 27 faded, 18 debug, 27 lab, 20 delayed retrieval/correction review, and 27 no-hint assessment. It retrieves heading hierarchy, prose boundaries, content models, and destinations while teaching meaning-driven emphasis and voice; exact quotation, attribution, citation, and visible-source roles; readable abbreviation expansion; page or article contact scope; valid machine date, time, offset, week, year, and duration values; conventional subscript and superscript boundaries; code roles versus preserved whitespace; distinct annotation, stale-content, edit, relevance, and presentation semantics; and exact ruby base, annotation, language, fallback, copy, and linearization relationships. One hundred ninety-nine interactions require changed cases, every learner action is exact-unique, and all thirteen new or retrieved concepts appear as primary evidence in every family.

The [landmarks, sections, and disclosure step design](research/courses/responsive-web-design-html-landmarks-sections-and-disclosure-step-design.json) adds 95 explicit interactions: 8 model, 32 workshop, 12 faded, 8 debug, 12 lab, 11 delayed retrieval/correction review, and 12 no-hint assessment. It retrieves content-derived heading hierarchy, exact content-model restrictions, and full fragment task behavior while teaching context-sensitive header, nav, main, aside, footer, and named-region exposure; article, section, div, and named-landmark grouping boundaries; first-summary labeling, boolean open state, native activation, justified or refused exclusive `name` grouping, and disclosure-versus-menu/tab/dialog boundaries; and native link, button, disclosure, and form-control behavior before custom roles. All 95 interactions require changed cases, every learner action is exact-unique, and all seven new or retrieved concepts appear as primary evidence in every family.

The [form data and controls step design](research/courses/responsive-web-design-html-forms-data-and-controls-step-design.json) adds 117 explicit interactions: 10 model, 40 workshop, 15 faded, 10 debug, 15 lab, 12 delayed retrieval/correction review, and 15 no-hint assessment. It retrieves native task behavior, language scopes, and page hierarchy while teaching minimized form purpose; form ownership; effective action, method, and submitter; exact ordered entry-list inclusion and omission; persistent visible labels, accessible names, descriptions, group context, instructions, and language; separate type, inputmode, autocomplete, name, constraint, current-value, and submitted-value evidence; fieldset and legend grouping, radio exclusivity, checkbox independence, option identity, state, keyboard behavior, and entries; and textarea defaults/current values, select and optgroup relationships, selected and disabled states, multiple entries, explicit button types, and destructive-reset refusal. All 117 interactions require changed cases, every learner action is exact-unique, and all eight new or retrieved concepts appear as primary evidence in every family.

The [form validation and recovery step design](research/courses/responsive-web-design-html-form-validation-and-recovery-step-design.json) adds 72 explicit interactions: 6 model, 24 workshop, 9 faded, 6 debug, 9 lab, 9 delayed retrieval/correction review, and 9 no-hint assessment. It retrieves persistent form information, exact boolean/enumerated/text attribute contracts, and complete native task behavior while teaching default/current/dirty values, focus, checkedness, selectedness, readonly, disabled, fieldset, reset, validation-candidate, and entry consequences; realistic required, length, type, range, step, and bounded-pattern constraints with individual validity flags, diverse valid and invalid cases, client bypass, and server authority; and complete multi-error recovery through specific text, control relationships, suggestions, summary links, notification and focus strategy, retained valid data, correction, contextual prevention or confirmation, redundant-entry avoidance, one authoritative request, no duplicate side effects, and perceivable success. All 72 interactions require changed cases, every learner action is exact-unique, and all six new or retrieved concepts appear as primary evidence in every family. Together the first twelve designs cover 84 of 266 families and 1,608 planned interactions with 1,366 changed cases. They are reviewable planning structure, not learner-facing copy, fictional state/error fixtures, authoritative receiver and side-effect ledger, executable checks, canonical answers, browser/assistive-technology evidence, or authored publication. The other 182 activity families remain undecomposed.

The [data tables step design](research/courses/responsive-web-design-html-data-tables-step-design.json) adds 95 explicit interactions: 8 model, 32 workshop, 12 faded, 8 debug, 12 lab, 11 delayed retrieval/correction review, and 12 no-hint assessment. It retrieves list selection, HTML content models, and persistent form information while teaching when intersecting row/column relationships justify a table; caption, column, row-group, row, header, data-cell, empty-state, coordinate, copy, and linearization structure; necessary versus decorative row and column spans with complete non-overlapping grid geometry; and exact simple, grouped, and irregular header context using the simplest verified association model. Responsive evidence preserves real table semantics, complete content, focus, page reflow, and bounded local grid scrolling without CSS-generated substitute labels. All 95 interactions require changed cases, every learner action is exact-unique, and all seven new or retrieved concepts appear as primary evidence in every family. Together the first thirteen designs cover 91 of 266 families and 1,703 planned interactions with 1,461 changed cases. They remain reviewable planning structure, not authored learner activities or publication. The other 175 activity families remain undecomposed.

The [accessibility models step design](research/courses/responsive-web-design-html-accessibility-models-step-design.json) adds 165 explicit interactions: 14 model, 56 workshop, 21 faded, 14 debug, 21 lab, 18 delayed retrieval/correction review, and 21 no-hint assessment. It retrieves landmark, image-purpose, form-recovery, table-header, and timed-media paths while teaching respectful task-specific people/barrier/strategy evidence; source-to-DOM-to-accessibility-API exposure; visible native-first accessible names with separate descriptions and exact precedence; independent rendering, DOM, accessibility-tree, focus, form, naming, and task-inclusion dimensions; meaningful source, reading, visual, accessibility, and focus orders with complete keyboard continuation; native-first ARIA boundaries whose semantic promises require complete behavior; and complementary claim-specific standards, automation, inspection, keyboard, zoom, responsive, assistive-technology, and observed-user evidence with explicit limits. Every interaction has a changed case, every learner action is exact-unique, and all twelve new or retrieved concepts appear as primary evidence in every family. Together the first fourteen designs cover 98 of 266 families and 1,868 planned interactions with 1,626 changed cases. They remain reviewable planning structure, not authored learner activities, conformance proof, or publication. The other 168 activity families remain undecomposed.

The [validation and changed-cases step design](research/courses/responsive-web-design-html-validation-and-changed-cases-step-design.json) adds 55 explicit interactions: 4 model, 16 workshop, 6 faded, 4 debug, 6 lab, 11 delayed retrieval/correction review, and 8 no-hint assessment. It retrieves parser recovery, complete-document structure, bounded accessibility evaluation, full form recovery, and every-cell table context while teaching checker messages as precise but limited conformance evidence; source, DOM, accessibility, network, console, request, server, responsive, and task surfaces selected by the hypotheses they can falsify; dependency-ordered minimal repair from first divergence; and invariant-driven changed cases across text, language, direction, quantity, order, resources, identifiers, relationships, input, state, keyboard, zoom, viewport, persistence, failure, retry, reset, and recovery. All 55 interactions require changed cases, every learner action is exact-unique, and all seven new or retrieved concepts appear as primary evidence in every family. Together the first fifteen designs cover 105 of 266 families and 1,923 planned interactions with 1,681 changed cases. They remain reviewable planning structure, not authored learner activities or runnable regressions. The other 161 activity families remain undecomposed.

The [independent HTML transfer step design](research/courses/responsive-web-design-html-independent-transfer-step-design.json) adds 45 explicit interactions: 4 model, 8 workshop, 4 faded, 4 debug, 6 lab, 11 delayed retrieval/correction review, and 8 no-hint assessment. It retrieves complete documents, link and fragment tasks, article and section relationships, full form recovery, every-cell table context, and invariant-driven changed cases while teaching unfamiliar stakeholder-brief interpretation; users, complete tasks, content authority and ownership, ambiguity, privacy, safety, failure, page, relationship, scope, and acceptance decisions; original construction from empty files; complete-process testing; causal evidence-backed revision; and professional defense of choices, alternatives, tradeoffs, evidence limits, unresolved risks, ownership, and next work. Every interaction has a changed case, every learner action is exact-unique, and all seven new or retrieved concepts appear as primary evidence in every family. The first sixteen HTML/tooling designs now cover all 112 planned HTML activity families and contain 1,968 explicit interactions with 1,726 changed cases. This closes detailed HTML planning only; it does not make any HTML activity authored, reviewed, flow-verified, piloted, approved, or published. The remaining 154 undecomposed families belong to the 22 CSS/design/responsive modules.

The [first CSS rules step design](research/courses/responsive-web-design-css-first-rules-step-design.json) adds 119 explicit interactions: 10 model, 40 workshop, 15 faded, 10 debug, 15 lab, 14 delayed retrieval/correction review, and 15 no-hint assessment. It retrieves HTML purpose, complete-document ownership, logical source and keyboard order, and hypothesis-selected validation and inspection before introducing saved CSS source through parsed rules, exact matches, computed values, rendering, checkpoints, and reload; semantic HTML versus author, browser, and user presentation; qualified-rule, declaration, property, value, delimiter, comment, at-rule, and bounded parser-recovery anatomy; external, internal, and inline application decisions plus resolved URL, response identity, media type, source order, parsed-rule, and computed evidence; and exact universal, type, reusable-class, and unique-identifier match sets with changed markup and proportional-hook defense. All 119 interactions require changed cases, all five new and four retrieved concepts appear as primary evidence in every family, every planned interaction mode appears in its family, all learner actions are exact-unique, and the local token-set audit finds no learner-action pair at or above 0.80 Jaccard similarity. The first seventeen designs now cover 119 of 266 planned activity families and contain 2,087 explicit interactions with 1,845 changed cases. These remain planning contracts: learner-facing copy, source fixtures, canonical answers, executable checks, independent reviews, persisted correction schedules, learner-flow verification, and pilots are still absent. The remaining 147 undecomposed families belong to 21 CSS/design/responsive modules.

The [selectors and states step design](research/courses/responsive-web-design-css-selectors-and-states-step-design.json) adds 117 explicit interactions: 10 model, 40 workshop, 15 faded, 10 debug, 15 lab, 12 delayed retrieval/correction review, and 15 no-hint assessment. It retrieves rule anatomy, exact basic-selector match sets, and content-derived heading hierarchy while teaching selector-list unions; descendant, child, adjacent-sibling, and subsequent-sibling relationships; attribute presence, exact, token, language-prefix, prefix, suffix, substring, and explicit case policies; real interaction, form, location, language, disclosure, and structural states; bounded logical selector functions; presentational pseudo-element fragments; and semantic list markers and counters. Changed DOM, attribute, state, fragment, ordering, insertion, copy, linearization, and named-environment cases prevent static keyword or screenshot evidence from passing. All 117 interactions require changed cases, all five new and three retrieved concepts appear as primary evidence in every family, every planned interaction mode appears in its family, every learner action is exact-unique, and the local token-set audit finds no learner-action pair at or above 0.80 Jaccard similarity. The first eighteen designs now cover 126 of 266 planned activity families and contain 2,204 explicit interactions with 1,962 changed cases. These remain planning contracts, not authored learner activities or publication. The remaining 140 undecomposed families belong to 20 CSS/design/responsive modules.

The [cascade, inheritance, and layers step design](research/courses/responsive-web-design-css-cascade-inheritance-and-layers-step-design.json) adds 119 explicit interactions: 10 model, 40 workshop, 15 faded, 10 debug, 15 lab, 14 delayed retrieval/correction review, and 15 no-hint assessment. It retrieves exact rule anatomy, loaded stylesheet evidence, attribute matching, and browser-owned pseudo-class states while teaching CSS value stages; inheritance and the `initial`, `inherit`, `unset`, `revert`, and `revert-layer` keywords; relevance, origin, importance, context, style attributes, layers, specificity, scope proximity, and source order; lexicographic A-B-C specificity including functional selectors; distinct normal and important layer ordering; bounded `@scope` eligibility and proximity with explicit Level 6 draft and implementation limits; and link-state behavior across simultaneous states, keyboard and pointer input, forced colors, focus obstruction, reader styles, and visited-history privacy. Every interaction requires a changed case; all five new and four retrieved concepts appear as primary evidence in every family; every planned interaction mode appears in its family; every learner action is exact-unique; and the local token-set audit finds no learner-action pair at or above 0.80 Jaccard similarity. The first nineteen designs now cover 133 of 266 planned activity families and contain 2,323 explicit interactions with 2,081 changed cases. These are research-backed planning contracts, not learner-facing copy, runnable fixtures, canonical grading, independent review, flow verification, pilots, approval, or publication. The remaining 133 undecomposed families belong to 19 CSS/design/responsive modules.

The [custom-properties step design](research/courses/responsive-web-design-css-custom-properties-step-design.json) adds 51 explicit interactions: 4 model, 16 workshop, 6 faded, 5 debug, 6 lab, 6 delayed retrieval/correction review, and 8 no-hint assessment. Its primary-source model separates permissive case-sensitive unregistered token streams, same-element computed lookup, default inheritance, missing and guaranteed-invalid values, valid empty values, nested fallbacks, consumer invalidity, shorthand substitution, and per-element dependency cycles from registered syntax, explicit inheritance, computationally independent initial values, typed computed validation, registration selection, cascade neutrality, generic `@supports` limits, and typed interpolation. Every interaction requires a changed case; both new and both retrieved concepts appear as primary evidence in every family; every planned mode appears; all learner actions are exact-unique; and the local token-set audit finds no pair at or above 0.80 Jaccard similarity. Decomposition exposed and repaired an impossible matrix contract whose four-interaction debug floor promised five distinct modes; its minimum and aggregate planning total are now five and 3,855. The first twenty designs cover 140 of 266 families and contain 2,374 explicit interactions with 2,132 changed cases. These remain planning contracts, not authored or published learning. The remaining 126 undecomposed families belong to 18 CSS/design/responsive modules.

The [display and box-model step design](research/courses/responsive-web-design-css-flow-display-and-box-model-step-design.json) adds 95 explicit interactions: 8 model, 32 workshop, 12 faded, 8 debug, 12 lab, 11 delayed retrieval/correction review, and 12 no-hint assessment. It retrieves exact rule precedence and HTML whitespace processing while teaching element-tree versus box-tree evidence; outer versus inner display; anonymous boxes and line fragments; bounded `none` and `contents` behavior; content, padding, border, margin, paint, hit, and layout-space boundaries; content-box and border-box equations including zero floors and unavoidable edge excess; sibling, parent-child, empty-box, positive, and negative margin collapse; and intent-driven formatting-context boundaries without overflow clipping tricks. Every interaction requires a changed case; all four new and three retrieved concepts appear as primary evidence in every family; every planned mode appears; all actions are exact-unique; and the local token-set audit finds zero pairs at or above 0.80 Jaccard similarity. The first twenty-one designs cover 147 of 266 families and contain 2,469 interactions with 2,227 changed cases. These remain planning contracts, not authored learning. The remaining 119 families belong to 17 CSS/design/responsive modules.

The [sizing, units, and overflow step design](research/courses/responsive-web-design-css-sizing-units-and-overflow-step-design.json) adds 183 explicit interactions: 16 model, 64 workshop, 24 faded, 16 debug, 24 lab, 15 delayed retrieval/correction review, and 24 no-hint assessment. Live primary-source review replaces secondary anchors with CSS Sizing Level 3 for intrinsic and extrinsic sizing, CSS Values Level 4 for length references and numeric math, CSS Logical Level 1 plus Writing Modes Level 4 for flow-relative mappings, CSS 2.2 for normal flow, and the 21 May 2026 Overflow Level 3 draft for clipping and scrolling. It removes Grid-only `minmax()` from this pre-Grid module and separates intrinsic keywords from numeric `min()`, `max()`, and `clamp()`. The design retrieves measured box areas and sizing edges while requiring normal-flow and intrinsic-contribution traces; CSS reference-pixel, physical-output, font-metric, and viewport reference frames; definite and indefinite percentage bases; parser, type, and property-grammar math evidence; active lower and upper bounds; LTR, RTL, and vertical logical-axis mapping; and content, error, focus, keyboard, touch, programmatic, nested-scroll, zoom, and print access. Every interaction requires a changed case; all eight new and two retrieved concepts appear as primary evidence in every family; every planned mode appears; all actions are exact-unique; and the local token-set audit finds zero pairs at or above 0.80 Jaccard similarity. The first twenty-two designs cover 154 of 266 families and contain 2,652 interactions with 2,410 changed cases. These remain planning contracts, not authored learning. The remaining 112 families belong to 16 CSS/design/responsive modules.

The [backgrounds, borders, and transforms step design](research/courses/responsive-web-design-css-backgrounds-borders-and-transforms-step-design.json) adds 156 explicit interactions: 12 model, 48 workshop, 24 faded, 18 debug, 18 lab, 16 delayed retrieval/correction review, and 20 no-hint assessment. Live review of the 16 December 2025 Backgrounds and Borders Level 3 Editor's Draft, the 30 November 2025 Transforms Level 1 Editor's Draft, Color Adjustment Level 1, and WAI decorative-image guidance exposed an inadequate two-concept umbrella. The repaired graph now separates background layer/list/fallback paint; position, size, repeat, attachment, origin, and clip geometry; border strokes and shorthand resets; normalized elliptical radii and clipping; nine-slice border images; ordered outer and inset shadow geometry; transform function composition; origin and reference-box percentages; and post-layout flow, overflow, client-rectangle, stacking, containing-block, focus, and hit effects. The design retrieves box areas, percentage bases, and contextual image purpose in every role. Every interaction requires a changed case; all nine new and three retrieved concepts appear as primary evidence in every family; all planned modes appear; all learner actions are exact-unique; and the local token-set audit finds no pair at or above 0.80 Jaccard similarity. The activity matrix now requires 384 model interactions, 1,520 workshop steps, and 3,947 pre-assessment interactions. The first twenty-three designs cover 161 of 266 families and contain 2,808 interactions with 2,566 changed cases. These remain planning contracts, not authored or published learning. The remaining 105 families belong to 15 CSS/design/responsive modules.

The [fonts and loading step design](research/courses/responsive-web-design-css-fonts-and-loading-step-design.json) replaces a four-topic umbrella with nine bounded font competencies and adds 140 explicit interactions: 12 model, 48 workshop, 18 faded, 12 debug, 18 lab, 14 delayed retrieval/correction review, and 18 no-hint assessment. Current primary evidence includes the 22 April 2026 CSS Fonts Level 4 Editor's Draft, 12 January 2026 CSS Font Loading Level 3 Editor's Draft, WOFF2 Recommendation, and official SIL Open Font License 1.1 guidance. The module separates family and glyph fallback; face matching and synthesis; `@font-face` sources and descriptors; deferred loading, display periods, readiness, and failure; exact license, notice, privacy, hosting, payload, and subset evidence; measured fallback metrics; variable axes and optical sizing; language-aware feature resolution; and color-font palette or emoji presentation. Inheritance and intrinsic sizing remain primary retrieved evidence in every role. Every interaction requires a changed case; all eleven new or retrieved concepts appear as primary evidence in every family; all planned modes appear; all learner actions are exact-unique; and the local token-set audit finds no pair at or above 0.80 Jaccard similarity. The graph now has 115 CSS/responsive and 198 total concepts, including 13 explicit modern extensions; the architecture has 148 retrieval edges covering 89 concepts; and the matrix plans 388 theory interactions, 1,536 workshop steps, and 3,988 pre-assessment interactions. The first twenty-four designs cover 168 of 266 families and contain 2,948 interactions with 2,706 changed cases. These are planning contracts, not authored or published learning. The remaining 98 families belong to 14 CSS/design/responsive modules.

The [competency evidence matrix](research/courses/responsive-web-design-competency-evidence-matrix.json) traces all 198 concepts through introduce/model, guided, faded, debug, independent transfer, familiar assessment, next relevant use, delayed retrieval, correction, parallel reassessment, and certification-strand references. Due points are adaptive scheduling seeds, not claims of one optimal interval. The current [research assessment specification](research/courses/responsive-web-design-assessment-blueprint.json) covers every repaired module and concept in 12 weighted strands and plans 50 balanced items per secure form across prediction, code/tree reading, causal debugging, changed cases, accessibility/design evidence, bounded repair, and defense. It requires at least three parallel forms, server-side canonical answers, five passing projects, correction before a different form, accessibility/security review, item analysis, and defensible standard setting. It sets no cut score and keeps credential issuance blocked because no items, forms, pilot data, scoring evidence, or standard-setting result exists.
