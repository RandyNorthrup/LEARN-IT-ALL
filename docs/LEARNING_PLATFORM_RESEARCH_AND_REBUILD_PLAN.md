...
The compiled and tested [v9-to-concept candidate alignment](research/courses/responsive-web-design-concept-alignment.json) preserves all 158 pinned source blocks and all 1,553 exact challenge identities without assigning guessed coverage. Its source snapshot captures 6,431,329 bytes across the exact pinned files plus paths, SHA-256 hashes, section inventories, 5,163 executable hint checks, and 1,365 question prompts. All previously inspected blocks remain exact. The complete [Attribute Selectors inspection](research/courses/responsive-web-design-attribute-selectors-inspection.md) adds four inspected blocks, 71 challenges, 19 question prompts, and 274 implementation checks. It corrects language/private-data confusion and absolute `!important` claims; rejects exact serialized-class recipes, copied visually-hidden declarations, global `z-index`, and fixed negative-offset balance-sheet tracing as target instruction; and replaces them with semantic contracts, complete matched-set prediction, DOM mutation, cascade provenance, accessible data-table tasks, changed cases, delayed retrieval, and transfer evidence. Current totals are 131 inspected blocks, 1,161 inspected challenges, 1,154 captured question prompts, and 3,822 inspected implementation checks. One hundred thirty-six blocks have block-specific candidate maps; 21 uninspected source blocks and the unavailable certification assessment retain exact source evidence with zero concept assignments. Six concepts remain explicitly unresolved. Eight concepts lacking enough pinned-v9 evidence remain explicit modern extensions. No broad, proportional, guessed, module-substitution, or compatibility mapping exists. Every record remains `candidate-review`; inspection of the remaining 27 blocks, independent reviews, original full evidence progression, assessment validation, runtime evidence, accessibility verification, and observed learner success remain blocking.

The compiled and tested [candidate research architecture](research/courses/responsive-web-design-course-architecture.json) translates verified candidate mappings into a reviewable 17-module cumulative sequence rather than copying the benchmark's 29-module navigation. It opens with `html-first-page`, requires a meaningful source edit by learner action two, then places `web-tooling-just-in-time` before documents and paths. It assigns every one of the 184 target concepts exactly once, maps only 136 bounded source objectives into modules, retains the other 22 source identities in an explicit non-specific inventory, names prior concepts to retrieve, bounds new complexity, and defines a continuing learner artifact. Five certification benchmarks become original stakeholder projects in mutual-aid, museum, public-library, emergency-information, and community-energy domains with distinct requirements, evidence, concepts, placements, and empty-start policies. All modules and projects remain `planned-not-authored`; the architecture remains `researching` and cannot drive learner-facing authoring until the remaining 27 inspections, all independent reviews, and activity-matrix gates close.
