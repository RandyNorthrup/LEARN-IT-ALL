...
The first concept-level [HTML research graph](research/courses/responsive-web-design-html-concepts.json) now defines 54 ordered concepts across eight proposed modules. Each concept has exact prerequisites, current primary-source anchors, an explicit misconception, at least two performance-evidence requirements, retained-use targets, and the full introduce, model, guided, faded, debug, retrieve, assess, and transfer progression. Every concept remains `researched-not-authored`; the graph itself remains `researching`. Automated validation rejects unknown or later prerequisites, incomplete stage progression, unknown source or module references, duplicate identities, and false sequence order. CSS/responsive research, reviewer closure, actual activity alignment, accessible editor verification, and beginner pilots remain required before the RWD dossier can advance.

The companion [CSS and responsive-design research graph](research/courses/responsive-web-design-css-concepts.json) adds 80 ordered concepts across CSS language and cascade, boxes and intrinsic sizing, typography/color/design, flex layout, grid/positioning, responsive systems, interaction/accessibility/motion, and independent transfer. The graph replaces device-name recipes with fluid defaults, content-derived breakpoints, container-aware components, changed-case evidence, and cumulative accessibility constraints. Together the two graphs contain 134 researched concept records. That number is research inventory—not authored lesson, practice, review, pilot, or publication evidence. Subject review, alignment to all 158 pinned v9 blocks, original activity design, runtime verification, and observed learner success remain blocking.
