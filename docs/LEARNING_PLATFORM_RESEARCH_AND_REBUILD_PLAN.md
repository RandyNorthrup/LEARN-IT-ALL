...
The companion [CSS and responsive-design research graph](research/courses/responsive-web-design-css-concepts.json) now defines 95 ordered concepts across CSS language and cascade, boxes and intrinsic sizing, typography/color/design, flex layout, grid/positioning, responsive systems, interaction/accessibility/motion, and independent transfer. Beyond list markers, link states, transforms, filters, evidence-backed user needs, and prototype evaluation, it now separates design briefs and handoff, essential baseline enhancement, hierarchical wayfinding, card action boundaries, progressive disclosure and proportionate registration, long-collection navigation, modal focus behavior, multi-step recovery, and cart correction. Current primary research includes CSS Syntax, Selectors, Cascade, Display, Backgrounds, Color Adjustment, WCAG, ARIA APG, WAI multi-page forms, W3C COGA, GOV.UK pagination and user research, MDN progressive enhancement, and current official handoff-tool evidence. The accepted `rwd-css-spec-graph`, `rwd-resilient-design-patterns`, and `rwd-current-design-tool-capabilities` decisions correct benchmark shortcuts, require complete behavior contracts, and reject stale vendor trivia. The graph replaces device-name and appearance recipes with fluid defaults, content-derived breakpoints, container-aware components, failure recovery, changed-case evidence, and cumulative accessibility constraints.

The generated and tested [v9-to-concept candidate alignment](research/courses/responsive-web-design-concept-alignment.json) preserves all 158 pinned source blocks and all 1,553 exact challenge identities without assigning guessed coverage. Its source snapshot captures 6,431,329 bytes across the exact pinned files plus paths, SHA-256 hashes, section inventories, 5,163 executable hint checks, and 1,365 question prompts. All previously inspected HTML, accessibility, tooling, and Basic CSS blocks remain exact. The complete new [Design for Developers inspection](research/courses/responsive-web-design-design-for-developers-inspection.md) adds all five design blocks, 23 challenges, and 103 prompts. It corrects intuitive contrast and PPI shortcuts, separates hierarchy from visit history, requires real card and modal interaction contracts, rejects automatic infinite scroll as a default, makes progress and cart state recoverable, treats registration as a proportionate task boundary, and replaces maintenance-only Adobe XD and mutable product trivia with durable brief, prototype, inspect, handoff, and implementation capabilities. This produces 78 inspected blocks, 463 inspected challenges, and 723 captured prompts. Ninety-three blocks have block-specific candidate maps; 64 uninspected blocks and the unavailable certification assessment retain exact source evidence with zero concept assignments. Six concepts remain explicitly unresolved. Seven concepts lacking enough pinned-v9 evidence remain explicit modern extensions. No broad or proportional fallback mapping exists, and tests reject guessed concepts on unmapped records. Every record remains `candidate-review`; inspection of the remaining 80 blocks, independent reviews, original full evidence progression, assessment validation, runtime evidence, accessibility verification, and observed learner success remain blocking.

The new generated and tested [candidate research architecture](research/courses/responsive-web-design-course-architecture.json) translates verified candidate mappings into a reviewable 17-module cumulative sequence rather than copying the benchmark's 29-module navigation or the current weak blueprint. It opens with `html-first-page`, requires a meaningful source edit by learner action two, then places `web-tooling-just-in-time` before documents and paths. It assigns every one of the 178 target concepts exactly once, maps only 93 bounded source objectives into modules, retains the other 65 source identities in an explicit unmapped inventory, names prior concepts to retrieve, bounds new complexity, and defines a continuing learner artifact. Five certification benchmarks become original stakeholder projects in mutual-aid, museum, public-library, emergency-information, and community-energy domains with distinct requirements, evidence, concepts, placements, and empty-start policies. All modules and projects remain `planned-not-authored`; the architecture remains `researching` and cannot drive learner-facing migration or authoring until the remaining 80 inspections, all independent reviews, and activity-matrix gates close.
