...
The first concept-level [HTML research graph](research/courses/responsive-web-design-html-concepts.json) now defines 57 ordered concepts across eight proposed modules. The scope includes the request/response/parse/render model, search and sharing metadata, and technical-authority verification that the first graph missed. Each concept has exact prerequisites, current primary-source anchors, an explicit misconception, at least two performance-evidence requirements, retained-use targets, and the full introduce, model, guided, faded, debug, retrieve, assess, and transfer progression. Every concept remains `researched-not-authored`; the graph itself remains `researching`. Automated validation rejects unknown or later prerequisites, incomplete stage progression, unknown source or module references, duplicate identities, and false sequence order. Reviewer closure, authored activity alignment, accessible editor verification, and beginner pilots remain required before the RWD dossier can advance.
