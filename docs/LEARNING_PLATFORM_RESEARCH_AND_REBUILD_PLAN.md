...
The generated and tested [v9-to-concept candidate alignment](research/courses/responsive-web-design-concept-alignment.json) now maps all 158 pinned source blocks and all 1,553 exact challenge identities to 162 of the 169 researched concepts. Its source snapshot captures 6,431,329 bytes across the exact pinned files plus paths, SHA-256 hashes, section inventories, 5,163 executable hint checks, and 1,365 question prompts. Semantic inspection exposed and repaired an evidence-import defect: current `quizzes` sections contain 810 prompts that the older `questions`-only parser had recorded as zero. All 23 Basic HTML blocks with 137 challenges, all 10 Semantic HTML blocks with 55 challenges, all eight Forms and Tables blocks with 53 challenges, the Survey lab, all 13 HTML Accessibility blocks with 55 challenges and 77 prompts, the cumulative HTML review, and all five Computer Basics blocks with 16 challenges and 82 prompts are now agent-inspected and block-specific. Computer/tooling inspection rejects stale browser-popularity and product-list trivia as target outcomes and bounds password, download, file-operation, and developer-tool instruction with current official security and environment guidance. This produces 61 inspected blocks, 318 inspected challenges, and 462 captured prompts; 83 blocks have block-specific candidate maps overall, 74 still carry explicit `module-fallback-overreach` blockers, and one certification container carries an unavailable-item blocker. Seven concepts that lack enough explicit pinned-v9 evidence—cascade layers, margin collapse, logical properties, subgrid, container queries and units, and print output—remain separately identified as modern course extensions. Every mapping and extension remains `candidate-review`; challenge-level inspection of the remaining 97 blocks, independent subject review, original I-G-F-R-A-T design, assessment review, runtime evidence, accessibility verification, and observed learner success remain blocking.

The new generated and tested [candidate research architecture](research/courses/responsive-web-design-course-architecture.json) translates those sources into a reviewable 17-module cumulative sequence rather than copying the benchmark's 29-module navigation or the current weak blueprint. It opens with `html-first-page`, requires a meaningful source edit by learner action two, then places `web-tooling-just-in-time` before documents and paths. It assigns every one of the 169 concepts exactly once, carries all 158 source-objective identities into module review, names prior concepts to retrieve, bounds new complexity, and defines a continuing learner artifact. Five certification benchmarks become original stakeholder projects in mutual-aid, museum, public-library, emergency-information, and community-energy domains with distinct requirements, evidence, concepts, placements, and empty-start policies. All modules and projects remain `planned-not-authored`; the architecture remains `researching` and cannot drive learner-facing migration or bulk authoring until the remaining 97 agent inspections, all independent reviews, and activity-matrix gates close.
