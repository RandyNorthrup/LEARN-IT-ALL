...
The first concept-level [HTML research graph](research/courses/responsive-web-design-html-concepts.json) now defines 79 ordered concepts across nine proposed modules. Source-file inspection found that the earlier graph had collapsed or omitted 12 beginner tooling competencies explicitly present in v9; complete Basic HTML inspection exposed replaced-content and media-rights models; complete Semantic HTML inspection separated dates, abbreviations, contact context, code and preformatted text, subscript and superscript, editorial annotations, and ruby annotations; and complete Forms and Tables inspection separated control-state/submission behavior and table-cell spans. Every added model has its own objective, prerequisites, misconceptions, changed-case evidence, and retention requirements. The emphasis model also distinguishes `em`, `strong`, `i`, and `b` from CSS-only presentation. Current WHATWG, MDN, WAI, CISA, U.S. Copyright Office, and Creative Commons evidence bounds these concepts. Legal instruction stays evidence-oriented and avoids claiming that educational, noncommercial, credited, or transformative use automatically establishes fair use. The target still begins with HTML: `web-tooling-just-in-time` follows the first meaningful edit instead of recreating the legacy 13-activity pre-HTML barrier. Every concept remains `researched-not-authored`; the graph remains `researching`.
...
The generated and tested [v9-to-concept candidate alignment](research/courses/responsive-web-design-concept-alignment.json) now maps all 158 pinned source blocks and all 1,553 exact challenge identities to 158 of the 165 researched concepts. Its source snapshot captures 6,431,329 bytes across the exact pinned files plus paths, SHA-256 hashes, section inventories, 5,163 executable hint checks, and 1,365 question prompts. Semantic inspection exposed and repaired an evidence-import defect: current `quizzes` sections contain 810 prompts that the older `questions`-only parser had recorded as zero. All 23 Basic HTML blocks with 137 challenges, all 10 Semantic HTML blocks with 55 challenges, and all eight Forms and Tables blocks with 53 challenges are now agent-inspected and block-specific. The Forms and Tables evidence includes 21 lecture prompts, a 40-prompt quiz, a 33-step form workshop, a 10-step table workshop, the independent table lab, and the complete review. This produces 41 inspected blocks and 245 inspected challenges; 70 blocks have block-specific candidate maps overall, 87 still carry explicit `module-fallback-overreach` blockers, and one certification container carries an unavailable-item blocker. Seven concepts that lack enough explicit pinned-v9 evidence—cascade layers, margin collapse, logical properties, subgrid, container queries and units, and print output—remain separately identified as modern course extensions. Every mapping and extension remains `candidate-review`; challenge-level inspection of the remaining 117 blocks, independent subject review, original I-G-F-R-A-T design, assessment review, runtime evidence, accessibility verification, and observed learner success remain blocking.

The new generated and tested [candidate research architecture](research/courses/responsive-web-design-course-architecture.json) translates those sources into a reviewable 17-module cumulative sequence rather than copying the benchmark's 29-module navigation or the current weak blueprint. It opens with `html-first-page`, requires a meaningful source edit by learner action two, then places `web-tooling-just-in-time` before documents and paths. It assigns every one of the 165 concepts exactly once, carries all 158 source-objective identities into module review, names prior concepts to retrieve, bounds new complexity, and defines a continuing learner artifact. Five certification benchmarks become original stakeholder projects in mutual-aid, museum, public-library, emergency-information, and community-energy domains with distinct requirements, evidence, concepts, placements, and empty-start policies. All modules and projects remain `planned-not-authored`; the architecture remains `researching` and cannot drive learner-facing migration or bulk authoring until the remaining 117 agent inspections, all independent reviews, and activity-matrix gates close.
