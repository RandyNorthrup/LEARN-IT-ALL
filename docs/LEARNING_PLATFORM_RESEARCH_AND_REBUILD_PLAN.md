...
All 54 courses are therefore `audit-required`. Existing generated content is not approved instruction and cannot publish. `content/v2/CONTENT_COMPLETE` remains absent and Lighthouse remains paused until the entire curriculum, obsolete-input removal, duplication, and learner-flow program is complete.
...
The earlier staged-retention decision was rejected. Publication is now deny-by-default through one explicit manifest containing zero course IDs. Home, catalog, paths, settings, both learner routes, the tracks API, and all course attempt/draft/hint APIs refuse the 54 audit-required generated courses. Direct old course routes and course APIs return 404; the learner UI shows an honest navigable empty state with no planned cards, fake hours, fake activity totals, or unusable filters. Git history is the only archive required for obsolete generated output. The active hard-cut task is to remove `blueprints/**`, `content/v2/courses/**`, their obsolete tests, and their runtime-index records after removing the remaining research-tool dependency on those files. No archive directory, renamed copy, compatibility route, or publication exception will replace them. Historical learner rows remain preserved exactly in one neutral evidence table because learner-owned evidence is data, not implementation.
...
The historical 54-blueprint baseline found 799 source entries but only 39 of 1,135 modules with source-objective IDs. Every generated blueprint lacked stable source identity, limitations, research-to-design decision links, review triggers, or complete objective traceability. Their former blueprint/content generators are gone, and the generated blueprints and course output are now publication-blocked pending deletion. Responsive Web Design is the first started dossier and remains `researching`; 53 dossiers are missing and zero courses are `researched`. See `docs/audits/RESEARCH_PROGRAM_BASELINE_2026-07-15.md`. These are historical gap measurements, not target architecture or research completion claims.
