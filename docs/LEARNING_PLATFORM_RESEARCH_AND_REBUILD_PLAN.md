...
LEARN-IT-ALL is not content-complete. The earlier checkpoint proved file inventory, schema validity, runtime packaging, and selected browser flows. It did not prove that a beginner could learn from the material. Direct inspection of the opening Responsive Web Design HTML activities found generic checkpoint prose, delayed coding, repeated instructions, token-presence grading, and insufficient explanation of HTML syntax. The same risk applies to every generator-produced course until audited.
...
The companion [CSS and responsive-design research graph](research/courses/responsive-web-design-css-concepts.json) adds 86 ordered concepts across CSS language and cascade, boxes and intrinsic sizing, typography/color/design, flex layout, grid/positioning, responsive systems, interaction/accessibility/motion, and independent transfer. It now explicitly includes list markers and counters, link-state sequencing, transforms, filters, evidence-backed user needs, and prototype evaluation. Current primary CSS research now separates Syntax Level 3, Selectors Level 4, Cascading and Inheritance Level 5, the exploratory Level 6 freshness watch, Display Level 3, and Backgrounds and Borders Level 3 instead of relying on one broad curriculum page. The accepted `rwd-css-spec-graph` decision corrects the benchmark's old four-position specificity shorthand and overbroad importance explanation, keeps CSS display separate from HTML meaning, and ties paint behavior to defined box areas. The graph replaces device-name recipes with fluid defaults, content-derived breakpoints, container-aware components, changed-case evidence, and cumulative accessibility constraints.

The generated and tested [v9-to-concept candidate alignment](research/courses/responsive-web-design-concept-alignment.json) preserves all 158 pinned source blocks and all 1,553 exact challenge identities without assigning guessed coverage. Its source snapshot captures 6,431,329 bytes across the exact pinned files plus paths, SHA-256 hashes, section inventories, 5,163 executable hint checks, and 1,365 question prompts. Semantic inspection exposed and repaired an evidence-import defect: current `quizzes` sections contain 810 prompts that the older `questions`-only parser had recorded as zero. All 23 Basic HTML blocks with 137 challenges, all 10 Semantic HTML blocks with 55 challenges, all eight Forms and Tables blocks with 53 challenges, the Survey lab, all 13 HTML Accessibility blocks with 55 challenges and 77 prompts, the cumulative HTML review, all five Computer Basics blocks with 16 challenges and 82 prompts, and all 12 Basic CSS blocks with 122 challenges and 158 prompts are now agent-inspected and block-specific. The new [Basic CSS source inspection](research/courses/responsive-web-design-basic-css-inspection.md) records the method, exact inventory, primary specifications, block findings, grading failures, and replacement activity contract. It rejects inline-block whitespace fitting, rigid proportions, negative-margin positioning, arbitrary recipe literals, unqualified new-tab behavior, appearance-only checks, and property-presence grading as target instruction. Computer/tooling inspection separately rejects stale browser-popularity and product-list trivia and bounds password, download, file-operation, and developer-tool instruction with current official security and environment guidance. This produces 73 inspected blocks, 440 inspected challenges, and 620 captured prompts. Ninety-one blocks have block-specific candidate maps; 66 uninspected blocks and the unavailable certification assessment carry exact source evidence but zero concept assignments. Six concepts remain explicitly unresolved until source inspection proves or rejects benchmark support. Seven concepts that lack enough explicit pinned-v9 evidence—cascade layers, margin collapse, logical properties, subgrid, container queries and units, and print output—remain separately identified as modern course extensions. The former broad module-fallback mechanism has been removed and tests reject guessed concepts on unmapped records. Every mapping and extension remains `candidate-review`; challenge-level inspection of the remaining 85 blocks, independent subject review, original introduce/model/guided/faded/debug/retrieve/assess/delayed-retain/transfer design, assessment review, runtime evidence, accessibility verification, and observed learner success remain blocking.

The new generated and tested [candidate research architecture](research/courses/responsive-web-design-course-architecture.json) translates verified candidate mappings into a reviewable 17-module cumulative sequence rather than copying the benchmark's 29-module navigation or the current weak blueprint. It opens with `html-first-page`, requires a meaningful source edit by learner action two, then places `web-tooling-just-in-time` before documents and paths. It assigns every one of the 169 target concepts exactly once, maps only 91 bounded source objectives into modules, retains the other 67 source identities in an explicit unmapped inventory, names prior concepts to retrieve, bounds new complexity, and defines a continuing learner artifact. Five certification benchmarks become original stakeholder projects in mutual-aid, museum, public-library, emergency-information, and community-energy domains with distinct requirements, evidence, concepts, placements, and empty-start policies. All modules and projects remain `planned-not-authored`; the architecture remains `researching` and cannot drive learner-facing migration or bulk authoring until the remaining 85 agent inspections, all independent reviews, and activity-matrix gates close.
...
Courses remain hidden or clearly labelled preview until their wave passes. A technically runnable preview is not published instruction.
