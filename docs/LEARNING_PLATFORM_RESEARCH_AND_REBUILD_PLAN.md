...
The [local-project tooling step design](research/courses/responsive-web-design-tooling-local-projects-step-design.json) adds 161 explicit interactions: 14 model, 56 workshop, 21 faded, 14 debug, 21 lab, 14 review, and 21 assessment. It introduces local resource evidence, accessible input parity, developer-tool roles and transfer limits, reversible bounded file operations, portable file identity, project-root and path invariants, and content-based search and inspection only after the learner already owns working HTML. One hundred thirty-three interactions require changed consequences. Together the first three designs cover 21 of 266 families and 504 planned interactions with 322 changed cases. They are reviewable planning structure, not learner-facing copy, simulator implementation, executable checks, canonical answers, or authored publication. The other 245 activity families remain undecomposed.
