...
The first concept-level [HTML research graph](research/courses/responsive-web-design-html-concepts.json) now defines 69 ordered concepts across nine proposed modules. A source-file inspection found that the earlier graph had collapsed or omitted 12 beginner competencies explicitly present in v9 computer basics: local resources, input ergonomics, internet access, account-sign-in safety, developer-tool roles, file operations, portable naming, project-folder organization, file types and search, browser installation and engines, browser/site/search-engine distinctions, and search refinement. Current MDN environment, software, editor, file, browsing, internet, and browser-loading guidance plus CISA phishing-resistant MFA guidance now bound those concepts. The target still begins with HTML: the new `web-tooling-just-in-time` module follows the first meaningful edit instead of recreating the legacy 13-activity pre-HTML barrier. Each concept has exact prerequisites, current primary-source anchors, an explicit misconception, at least two performance-evidence requirements, retained-use targets, and the full introduce, model, guided, faded, debug, retrieve, assess, and transfer progression. Every concept remains `researched-not-authored`; the graph remains `researching`.
...
The generated and tested [v9-to-concept candidate alignment](research/courses/responsive-web-design-concept-alignment.json) now maps all 158 pinned source blocks and all 1,553 exact challenge identities to 148 of the 155 researched concepts. Its source snapshot captures 6,431,329 bytes across the exact pinned files plus paths, SHA-256 hashes, section inventories, 5,163 executable hint checks, and 555 question prompts. That evidence prevents identity and source drift, but does not pretend automated capture equals pedagogical review. Seven opening blocks containing exactly 61 source challenges are agent-inspected and narrowed; 48 blocks have block-specific candidate maps overall, 109 still carry explicit `module-fallback-overreach` blockers, and one certification container carries an unavailable-item blocker. Seven concepts that lack enough explicit pinned-v9 evidence—cascade layers, margin collapse, logical properties, subgrid, container queries and units, and print output—remain separately identified as modern course extensions backed by current primary sources. Every mapping and extension remains `candidate-review`; challenge-level inspection, independent subject review, original I-G-F-R-A-T design, assessment review, runtime evidence, accessibility verification, and observed learner success remain blocking.

The new generated and tested [candidate research architecture](research/courses/responsive-web-design-course-architecture.json) translates those sources into a reviewable 17-module cumulative sequence rather than copying the benchmark's 29-module navigation or the current weak blueprint. It opens with `html-first-page`, requires a meaningful source edit by learner action two, then places `web-tooling-just-in-time` before documents and paths. It assigns every one of the 155 concepts exactly once, carries all 158 source-objective identities into module review, names prior concepts to retrieve, bounds new complexity, and defines a continuing learner artifact. Five certification benchmarks become original stakeholder projects in mutual-aid, museum, public-library, emergency-information, and community-energy domains with distinct requirements, evidence, concepts, placements, and empty-start policies. All modules and projects remain `planned-not-authored`; the architecture remains `researching` and cannot drive learner-facing migration or bulk authoring until the remaining 151 agent inspections, all independent reviews, and activity-matrix gates close.
...
7. `npm run build`;
8. tablet and desktop browser verification of real learner flows plus the accessible phone handoff;
...
- all local technical gates and tablet/desktop learner flows pass;
