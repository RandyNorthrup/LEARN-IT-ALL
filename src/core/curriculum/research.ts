...
export const ConceptResearchGraphSchema = z
  ...
  .superRefine((graph, context) => {
    ...
    const moduleOrder = new Map(
      graph.moduleIds.map((moduleId, index) => [moduleId, index]),
    );
    ...
    const normalizedObjectives = graph.concepts.map((concept) =>
      normalize(concept.objective),
    );
    ...
    if (
      new Set(normalizedMisconceptions).size !== normalizedMisconceptions.length
    ) {
      context.addIssue({
        code: "custom",
    ...
    for (const [conceptIndex, concept] of graph.concepts.entries()) {
      ...
      for (const moduleId of concept.retainedInModuleIds) {
        if (!moduleIds.has(moduleId)) {
          ...
          (moduleOrder.get(moduleId) ?? -1) <
          (moduleOrder.get(concept.moduleId) ?? -1)
        ) {
          context.addIssue({
            code: "custom",
...
export const ExternalObjectiveConceptAlignmentSchema = z
  ...
  .superRefine((matrix, context) => {
    const objectiveIds = matrix.alignments.map(
      (alignment) => alignment.objectiveId,
    );
    const blockSlugs = matrix.alignments.map(
      (alignment) => alignment.sourceBlockSlug,
    );
    const extensionIds = matrix.courseExtensions.map(
      (extension) => extension.extensionId,
    );
    const inventoryScopeIds = matrix.conceptInventories.map(
      (inventory) => inventory.scopeId,
    );
    ...
    rejectDuplicates(objectiveIds, "Duplicate aligned source objective IDs", [
      "alignments",
    ]);
    rejectDuplicates(blockSlugs, "Duplicate aligned source block slugs", [
      "alignments",
    ]);
    rejectDuplicates(extensionIds, "Duplicate course extension IDs", [
      "courseExtensions",
    ]);
    rejectDuplicates(
      inventoryScopeIds,
      "Duplicate concept inventory scope IDs",
      ["conceptInventories"],
    );
    rejectDuplicates(
      inventoryConceptIds,
      "Concept appears in multiple inventories",
      ["conceptInventories"],
    );
    rejectDuplicates(
      extensionConceptIds,
      "Concept appears in multiple course extensions",
      ["courseExtensions"],
    );
    rejectDuplicates(
      matrix.unresolvedConceptIds,
      "Concept appears multiple times as unresolved",
      ["unresolvedConceptIds"],
    );
    rejectDuplicates(
      sourceChallengeIds,
      "Challenge appears in multiple alignment evidence sets",
      ["alignments"],
    );
    ...
    for (const [
      inventoryIndex,
      inventory,
    ] of matrix.conceptInventories.entries()) {
    ...
    for (const [alignmentIndex, alignment] of matrix.alignments.entries()) {
      ...
      if (
        alignment.sourceEvidence.challengeIds.length !==
          alignment.sourceChallengeCount ||
        alignment.sourceEvidence.sourceFileSha256s.length !==
          alignment.sourceChallengeCount
      ) {
        context.addIssue({
          code: "custom",
      ...
      if (alignment.mappingBasis === "unmapped-source") {
        ...
        if (
          alignment.reviewFindings.every(
            (finding) =>
              finding.code !== "source-inspection-required" ||
    ...
    for (const [
      extensionIndex,
      extension,
    ] of matrix.courseExtensions.entries()) {
      rejectDuplicates(
    ...
    if (matrix.status === "approved") {
      ...
      if (
        ...
        matrix.courseExtensions.some(
          (extension) => extension.state !== "approved",
        )
      ) {
        context.addIssue({
          code: "custom",
          message:
            "Approved alignment requires every objective and extension approved",
        });
      }
    }
  });
...
export const ResearchCourseArchitectureSchema = z
  ...
  .superRefine((architecture, context) => {
    const moduleById = new Map(
      architecture.modules.map((module) => [module.id, module]),
    );
    const moduleOrder = new Map(
    ...
    const moduleConceptIds = architecture.modules.flatMap(
      (module) => module.conceptIds,
    );
    ...
    const projectDomains = architecture.projects.map(
      (project) => project.scenarioDomain,
    );
    ...
    duplicateIssue(
      ...
      "Duplicate architecture module IDs",
      ["moduleIds"],
    );
    duplicateIssue(
    ...
    duplicateIssue(
      architecture.conceptIds,
      "Duplicate architecture concept inventory IDs",
      ["conceptIds"],
    );
    duplicateIssue(
      moduleConceptIds,
      "Concept is assigned to multiple architecture modules",
      ["modules"],
    );
    duplicateIssue(
      architecture.sourceObjectiveIds,
      "Duplicate architecture source objective IDs",
      ["sourceObjectiveIds"],
    );
    duplicateIssue(
    ...
    duplicateIssue(projectIds, "Duplicate architecture project IDs", [
      "projects",
    ]);
    duplicateIssue(
      projectDomains,
      "Architecture projects repeat a scenario domain",
      ["projects"],
    );
    duplicateIssue(
      projectSourceObjectiveIds,
      "Certification project source objective is reused",
      ["projects"],
    );
    ...
    if (
      ...
      context.addIssue({
        code: "custom",
        message:
          "Architecture module inventory does not match module record order",
        path: ["moduleIds"],
      });
    }
    if (
      architecture.entryContract.openingModuleId !== architecture.moduleIds[0]
    ) {
      context.addIssue({
        code: "custom",
    ...
    if (
      ...
      context.addIssue({
        code: "custom",
        message:
          "Architecture source objectives cannot be both mapped and unmapped",
    ...
    for (const [moduleIndex, module] of architecture.modules.entries()) {
      ...
      duplicateIssue(
        module.conceptIds,
        `Architecture module ${module.id} repeats concepts`,
    ...
    for (const [projectIndex, project] of architecture.projects.entries()) {
      ...
      if (
        project.sourceObjectiveIds.length +
          project.unmappedSourceObjectiveIds.length ===
        0
      ) {
        context.addIssue({
          code: "custom",
      ...
      duplicateIssue(
        project.conceptIds,
        `Architecture project ${project.id} repeats concepts`,
        ["projects", projectIndex, "conceptIds"],
      );
    ...
    if (architecture.status === "approved") {
      if (
        ...
        architecture.unmappedSourceObjectiveIds.length > 0
      ) {
        context.addIssue({
          code: "custom",
          message:
            "Approved architecture cannot retain gaps or unmapped source objectives",
        });
      }
      if (
        architecture.modules.some(
          (module) => module.currentState !== "reviewed",
        ) ||
        architecture.projects.some(
          (project) => project.currentState !== "reviewed",
        )
      ) {
        context.addIssue({
          code: "custom",
          message:
            "Approved architecture requires every module and project reviewed",
        });
      }
    }
  });
...
function addReferenceIssues(
  ...
  const decisionById = new Map(
    value.decisions.map((decision) => [decision.id, decision]),
  );
    ...
    for (const claim of source.claims) {
      for (const decisionId of claim.decisionIds) {
        if (!decisionIds.has(decisionId)) {
          ...
          !decisionById.get(decisionId)?.sourceIds.includes(source.id)
        ) {
          context.addIssue({
            code: "custom",
...
export type PlatformResearchRegister = z.infer<
  typeof PlatformResearchRegisterSchema
>;
...
>;
export type ResearchCourseArchitecture = z.infer<
  typeof ResearchCourseArchitectureSchema
>;
...
export function auditCourseResearch(
  ...
  if (sources.some((source) => !source.id)) {
    findings.push({
      ...
      message:
        "Source records need stable IDs before objectives and decisions can cite them.",
    });
  }
  if (sources.some((source) => !source.limitations?.length)) {
    findings.push({
      ...
      message:
        "Source records do not yet state evidence limits or uncertainty.",
    });
  }
  if (sources.some((source) => !source.decisionIds?.length)) {
    findings.push({
      ...
      message:
        "Source records do not yet trace accepted evidence into design decisions.",
    });
  }
  if (sources.some((source) => !source.nextReview)) {
    findings.push({
      ...
      message:
        "Source records do not yet define expiry dates or upstream-change triggers.",
    });
  }
