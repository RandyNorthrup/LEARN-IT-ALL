...
describe('research contracts', () => {
  ...
  it('decomposes first CSS rules into 119 evidence-bound interactions', () => {
    const design = ResearchModuleStepDesignSchema.parse(
      readJson(
        path.join(
          repositoryRoot,
          'docs/research/courses/responsive-web-design-css-first-rules-step-design.json'
        )
      )
    );
    const architecture = ResearchCourseArchitectureSchema.parse(
  ...
  it('rejects selector-match evidence without a changed case', () => {
    const missingChangedCase = structuredClone(
      readJson(
        path.join(
          repositoryRoot,
    ...
    const selectorInteraction = missingChangedCase.activityDesigns
      .flatMap((activity) => activity.interactions)
      .find((interaction) => interaction.evidence.kind === 'selector-match-set');
    if (!selectorInteraction) {
