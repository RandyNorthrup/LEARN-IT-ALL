...
function courseDossier(status: "researching" | "approved" = "researching") {
  const questions = ["question-one", "question-two", "question-three"].map(
    (id, index) => ({
      id,
      track:
        index === 0
          ? ("subject-scope" as const)
          : ("learning-science" as const),
  ...
  const decisions = [
    {
      ...
      validation: [
        "A changed learner task and reviewer evidence must pass before approval.",
      ],
    },
  ];
  return {
    ...
    scopeIncludes: [
      "Bounded concepts",
      "Guided practice and correction",
      "Independent transfer",
    ],
    scopeExcludes: [
      "Unsafe real-world actions outside an authorized learning environment",
    ],
    authenticTasks: [
    ...
    safetyBoundaries: [
      "Learner artifacts never execute on the application host.",
    ],
    sources,
    decisions,
    coverage: [],
    openQuestions: [
      "Observed learner evidence remains required before research closes.",
    ],
    reviews: {
...
describe("research contracts", () => {
  it("accepts the real platform research register with traceable sources and decisions", () => {
    ...
    expect(
      new Set(register.questions.map((question) => question.track)),
    ).toHaveLength(11);
    expect(
      register.sources.every(
        (source) => source.claims[0].limitations.length > 0,
      ),
    ).toBe(true);
  ...
  it("keeps the required platform research artifacts reviewable in the repository", () => {
    ...
    for (const artifact of requiredArtifacts) {
      const content = readFileSync(
        path.join(repositoryRoot, "docs/research", artifact),
        "utf8",
      );
      expect(
        content.length,
        `${artifact} must contain substantive research`,
      ).toBeGreaterThan(2_000);
      expect(
        content,
        `${artifact} must record limitations or unresolved evidence`,
      ).toMatch(/limit|unresolved|open item|residual risk|current blocker/i);
    }
  });
  ...
  it("records the inspected Responsive Web Design beginner sequence and its blockers", () => {
    const content = readFileSync(
      path.join(
        repositoryRoot,
        "docs/research/courses/responsive-web-design-beginner-sequence.md",
      ),
      "utf8",
    );
    ...
    expect(content).toContain(
      "The first meaningful edit must occur within two learner actions",
    );
  ...
  it("captures every exact pinned v9 challenge source as reproducible research evidence", () => {
    const snapshot = ExternalCurriculumEvidenceSnapshotSchema.parse(
      readJson(
        path.join(repositoryRoot, "references/freecodecamp-rwd-v9.json"),
      ),
    );
    ...
    expect(new Set(challenges.map((challenge) => challenge.id))).toHaveLength(
      1553,
    );
    expect(
      challenges.every(
    ...
    expect(
      challenges.reduce(
        (total, challenge) =>
          total + challenge.sourceEvidence.quizQuestionCount,
        0,
      ),
    ).toBe(1_365);
  });
  ...
  it("rejects corrupted pinned-v9 source evidence", () => {
    ...
    brokenHash.chapters[0].modules[0].blocks[0].challengeOrder[0].sourceEvidence.sha256 =
      "not-a-source-hash";
    expect(
      ExternalCurriculumEvidenceSnapshotSchema.safeParse(brokenHash).success,
    ).toBe(false);
    ...
    const result =
      ExternalCurriculumEvidenceSnapshotSchema.safeParse(brokenPath);
    expect(result.success).toBe(false);
    expect(
      result.error?.issues.some((issue) =>
        issue.message.includes("path differs from block"),
      ),
    ).toBe(true);
  });
  ...
  it("keeps the Responsive Web Design HTML concept graph prerequisite-ordered and source-backed", () => {
    ...
    const dossierSourceIds = new Set(
      dossier.sources.map((source) => source.id),
    );
    ...
    expect(
      graph.concepts.find(
        (concept) => concept.id === "html-media-rights-licensing",
      )?.sourceAnchors,
    ).toHaveLength(3);
    expect(
      dossier.decisions.find(
        (decision) => decision.id === "rwd-media-rights-evidence",
      ),
    ).toMatchObject({
      status: "accepted",
      sourceIds: [
    ...
    expect(
      dossier.decisions.find(
        (decision) => decision.id === "rwd-accessibility-cumulative",
      )?.sourceIds,
    ).toEqual(
      expect.arrayContaining([
    ...
    expect(
      graph.concepts.every(
        (concept) => concept.currentState === "researched-not-authored",
      ),
    ).toBe(true);
    expect(
      graph.sourceIds.every((sourceId) => dossierSourceIds.has(sourceId)),
    ).toBe(true);
  ...
  it("keeps the Responsive Web Design CSS and responsive graph ordered and source-backed", () => {
    ...
    const conceptCounts = Object.fromEntries(
      graph.moduleIds.map((moduleId) => [
        moduleId,
        graph.concepts.filter((concept) => concept.moduleId === moduleId)
          .length,
      ]),
    );
    ...
    expect(
      dossier.decisions.find(
        (decision) => decision.id === "rwd-css-spec-graph",
      ),
    ).toMatchObject({
      status: "accepted",
      sourceIds: [
    ...
    expect(
      Object.fromEntries(
        [
          ...
          conceptId,
          graph.concepts.find((concept) => concept.id === conceptId)
            ?.sourceAnchors[0].sourceId,
        ]),
      ),
    ).toEqual({
    ...
    expect(
      graph.concepts.find(
        (concept) => concept.id === "css-type-class-id-selectors",
      )?.title,
  ...
  it("records the complete pinned Basic CSS inspection and its replacement constraints", () => {
    ...
    expect(content).toContain(
      "Specificity is not a decimal or four-position score",
    );
    expect(content).toContain(
      "fitting two inline-block columns through source-whitespace",
    );
    expect(content).toContain(
      "Source text or declaration presence is never sufficient evidence",
    );
  ...
  it("records and bounds the complete Design for Developers inspection", () => {
    ...
    expect(
      dossier.decisions.find(
        (decision) => decision.id === "rwd-resilient-design-patterns",
      ),
    ).toMatchObject({ status: "accepted" });
    expect(
      dossier.decisions.find(
        (decision) => decision.id === "rwd-current-design-tool-capabilities",
      ),
    ).toMatchObject({
      status: "accepted",
      sourceIds: [
  ...
  it("keeps all five Design for Developers source blocks exact and agent-inspected", () => {
    ...
    expect(
      designAlignments.reduce(
        (total, alignment) =>
          total + alignment.sourceEvidence.quizQuestionCount,
        0,
      ),
    ).toBe(103);
    expect(
    ...
    expect(
      Object.fromEntries(
        designAlignments.map((alignment) => [
  ...
  it("records and bounds the complete Absolute and Relative Units inspection", () => {
    ...
    expect(content).toContain(
      "four blocks, eight challenges, 35 question prompts",
    );
    expect(content).toContain(
      "default `v*` viewport units use the large viewport",
    );
    ...
    expect(content).toContain("The remaining 76 source blocks");
    expect(content.length).toBeGreaterThan(15_000);
    expect(graph.concepts.map((concept) => concept.id)).toContain(
      "css-calculated-value-math",
    );
    expect(
      dossier.decisions.find(
        (decision) => decision.id === "rwd-length-reference-frame-evidence",
      ),
    ).toMatchObject({
      status: "accepted",
      sourceIds: [
  ...
  it("aligns every pinned v9 block to known concepts without hiding modern extensions", () => {
        ...
        chapter.modules.flatMap((module) =>
          module.blocks.map(
            (block) =>
              [
                block.objectiveId,
                { ...block, sourceModuleId: module.id },
              ] as const,
          ),
        ),
      ),
    ...
    expect(
      matrix.alignments.every(
        (alignment) => alignment.state === "candidate-review",
      ),
    ).toBe(true);
    expect(
      matrix.courseExtensions.flatMap((extension) => extension.conceptIds),
    ).toHaveLength(7);
    expect(
      matrix.conceptInventories.map((inventory) => inventory.conceptCount),
    ).toEqual([83, 96]);
    expect(
      matrix.alignments.filter(
    ...
    expect(
      matrix.alignments
        .filter((alignment) =>
          inspectedOpeningBlocks.includes(alignment.sourceBlockSlug),
        )
        .every(
          (alignment) =>
            alignment.inspectionState === "agent-inspected" &&
    ...
    expect(
      Object.fromEntries(
        semanticHtmlAlignments.map((alignment) => [
          alignment.sourceBlockSlug,
          alignment.conceptIds,
        ]),
      ),
    ...
    expect(
      semanticHtmlAlignments.find(
        (alignment) => alignment.sourceBlockSlug === "quiz-semantic-html",
    ...
    expect(
      Object.fromEntries(
        formsTablesAlignments.map((alignment) => [
          alignment.sourceBlockSlug,
          alignment.conceptIds,
        ]),
      ),
    ...
    expect(
      Object.fromEntries(
        basicCssAlignments.map((alignment) => [
          alignment.sourceBlockSlug,
          alignment.conceptIds,
        ]),
      ),
    ...
    expect(
      new Set(matrix.alignments.map((alignment) => alignment.mappingBasis)),
    ).not.toContain("module-fallback");
    expect(
      matrix.alignments
        .filter(
          (alignment) => alignment.mappingBasis !== "block-specific-source",
    ...
    for (const alignment of matrix.alignments) {
      ...
      expect(alignment.sourceEvidence.sourceFileSha256s).toEqual(
        sourceEvidence?.challengeOrder.map(
          (challenge) => challenge.sourceEvidence.sha256,
        ),
      );
    ...
    expect(
      matrix.alignments.find(
        (alignment) =>
          alignment.sourceBlockSlug === "lab-debug-camperbots-profile-page",
      )?.conceptIds.length,
    ).toBeLessThanOrEqual(6);
    expect(
      matrix.alignments.find(
        (alignment) =>
          alignment.sourceBlockSlug === "lab-debug-pet-adoption-page",
      )?.conceptIds.length,
    ).toBeLessThanOrEqual(6);

    expect(
      matrix.conceptInventories.flatMap((inventory) => inventory.conceptIds),
    ).toEqual(
  ...
  it("keeps uninspected source blocks explicitly unmapped and blocks guessed coverage", () => {
    ...
    if (!unmapped)
      throw new Error(
        "Expected at least one explicitly unmapped source block.",
      );
    ...
    expect(
      result.error?.issues.some((issue) =>
        issue.message.includes("assigns guessed concepts"),
      ),
    ).toBe(true);
  });
  ...
  it("rejects treating one concept as benchmark evidence and a modern extension", () => {
    ...
    const broken = structuredClone(matrix);
    broken.courseExtensions[0].conceptIds.push(
      broken.alignments[0].conceptIds[0],
    );
    ...
    expect(
      result.error?.issues.some((issue) =>
        issue.message.includes("both benchmark coverage"),
      ),
    ).toBe(true);
  });
  ...
  it("turns the RWD research into a cumulative beginner-first course architecture", () => {
    ...
    expect(architecture.conceptIds).toEqual(
    ...
    expect(
      architecture.modules.every(
        (module) => module.currentState === "planned-not-authored",
      ),
    ).toBe(true);
    expect(
      architecture.projects.every(
        (project) => project.currentState === "planned-not-authored",
      ),
    ).toBe(true);
    expect(
      new Set(architecture.projects.map((project) => project.scenarioDomain)),
    ).toHaveLength(5);
    expect(
      new Set(
    ...
    for (const concept of concepts) {
      ...
      const ownerModule = architecture.modules.find(
        (module) => module.id === owner,
      );
  ...
  it("rejects a research architecture that assigns project work before instruction", () => {
    ...
    expect(
      result.error?.issues.some((issue) =>
        issue.message.includes("before instruction"),
      ),
    ).toBe(true);
  });
  ...
  it("rejects a concept graph whose prerequisite appears later in the sequence", () => {
    ...
    expect(
      result.error?.issues.some((issue) =>
        issue.message.includes("non-earlier prerequisite"),
      ),
    ).toBe(true);
  });
  ...
  it("rejects research claims that cite missing decisions", () => {
    ...
    expect(
      result.error?.issues.some((issue) =>
        issue.message.includes("unknown decision"),
      ),
    ).toBe(true);
  });

  it("allows honest in-progress dossiers but blocks unsupported approval", () => {
    expect(CourseResearchDossierSchema.safeParse(courseDossier()).success).toBe(
      true,
    );

    const result = CourseResearchDossierSchema.safeParse(
      courseDossier("approved"),
    );
    expect(result.success).toBe(false);
