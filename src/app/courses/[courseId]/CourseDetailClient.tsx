...
import { ArrowLeft, BookOpen, CheckCircle2, Clock, Code2, Lock, Unlock } from 'lucide-react';
...
export default function CourseDetailClient({ course, progress }: CourseDetailClientProps) {
  const [isStructuredMode, setIsStructuredMode] = useState(true);

  useEffect(() => {
    setIsStructuredMode(localStorage.getItem('learningMode') !== 'open');
  }, []);

  const toggleLearningMode = () => {
    const nextMode = !isStructuredMode;
    setIsStructuredMode(nextMode);
    localStorage.setItem('learningMode', nextMode ? 'structured' : 'open');
  };
  ...
  const isLessonLocked = (chapterIndex: number, lessonIndex: number): boolean => {
    ...
    if (lessonIndex === 0) {
      ...
      return !prevChapter.lessons.every((l) =>
        progress.completedLessons.includes(l.replace('.md', ''))
      );
    }
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mb-6 rounded-2xl bg-white shadow-xl p-6">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-lg font-bold text-gray-900 mb-1">Learning Mode</h2>
              ...
              <div className="mt-4 flex gap-4 text-xs">
                <div
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg ${isStructuredMode ? 'bg-blue-100 text-blue-800 font-semibold' : 'bg-gray-100 text-gray-600'}`}
                >
                  <Lock className="h-4 w-4" />
                  Structured Mode
                </div>
                <div
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg ${!isStructuredMode ? 'bg-green-100 text-green-800 font-semibold' : 'bg-gray-100 text-gray-600'}`}
                >
                    ...
                    <div className="divide-y divide-gray-100">
                      {chapter.lessons.map((lesson, lessonIndex) => {
                        ...
                        return (
                          <Link
                            key={lesson}
                            href={isLocked ? '#' : `/courses/${course.id}/lessons/${lessonId}`}
                          ...
                          >
                            <div className="flex items-center flex-1">
                              ...
                              <span
                                className={`text-sm ${isLocked ? 'text-gray-700' : 'text-gray-900 font-medium'}`}
                              >
                                Lesson {chapterIndex + 1}.{lessonIndex + 1}:{' '}
                                {lessonId.replace(/^lesson-\d+-/, '').replace(/-/g, ' ')}
                              </span>
                            </div>
                            {isLocked && (
                    ...
                    <div className="flex items-center mb-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-amber-700">
                          Comprehensive assessment of all concepts
                        </p>
                      </div>
                    </div>
                    <Link
